...
// Dummy hash function
pub fn hash(bitstring: Vec<bool>) -> types::GroupElementFq {
    let mut hash = 0;
...
pub fn hash_two (r: types::GroupElementFp, msg: types::Message) -> types::GroupElementFq {
    let mut bitstring = to_bits(r);
    bitstring.extend(msg.get_bitstring());
    hash(bitstring)
}

pub fn sign(sk: types::GroupElementFq, nonce: types::GroupElementFq, msg: types::Message) -> (types::GroupElementFq, types::GroupElementFq) {
    let r = exp(config::G, nonce);
    let e =  hash_two(r, msg);
    let xe = montgomery::multiply_q(e, sk);
    let mut s = (nonce as u32) + (xe as u32);
...
pub fn sign_random_nonce(sk: types::GroupElementFq, msg: types::Message) -> (types::GroupElementFq, types::GroupElementFq) {
...
mod tests {
    ...
    fn test_schnorr() {
        ...
        let (e, s) = sign_random_nonce(sk, msg);
